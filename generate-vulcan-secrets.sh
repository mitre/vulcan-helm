...
generate_secrets () {
	if ! grep -qF "POSTGRES_PASSWORD" env/vulcan-secrets.yaml; then
		...
		echo -e "POSTGRES_PASSWORD: $PASSWORD" >> env/vulcan-secrets.yaml
		echo -e "DATABASE_URL: postgres://vulcanpostgres:$PASSWORD@postgres/vulcan_psql_production" >> env/vulcan-secrets.yaml
	else
...
if [ -f env/vulcan-secrets.yaml ]; then
	...
	echo -e "${RED}If you would like to regenerate your secrets, please delete env/vulcan-secrets.yaml and re-run the script.${NC}"
	echo  ""
	generate_secrets
else
